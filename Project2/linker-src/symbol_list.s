...
addr_for_symbol_loop:
	beq $s0 $0 addr_fail #check if NULL
        lw $a0 0($s0) #name for current SymbolList
        lw $a1 4($sp) #set a1 to name we are looking for
        jal streq
        beq $v0 $0 addr_success  #char names match
        lw $s0 8($s0)  #grab next pointer
        j addr_for_symbol_loop #iterate
        
addr_fail:
	addiu $v0 $0 -1 #set v0 to -1
	#addiu $v0 $v0 -1 #set v0 to -1
	j addr_end

addr_success:
	lw $v0 4($s0) #get addr from link
	
addr_end:
	lw $ra 0($sp)  #reset stack.  Set old values back
	lw $s0 8($sp)
	addiu $sp $sp 12
...
add_to_list:    
        ...
        sw $a0 4($sp) #prt to list
        sw $a1 8($sp) #pointer to symbol name
        sw $a2 12($sp) #addr of sym
        sw $s0 16($sp)  #using s0 to store new node pointer
        jal new_node  #returns new node pointer
        addu $s0 $v0 $0  
        lw $a0 8($sp) #get pointer to string
        jal copy_of_str
        sw $v0 0($s0)  #save string pointer in new node
        lw $t0 12($sp) #get node addr
        sw $t0 4($s0) #save addr in new node
        lw $t0 4($sp) #get head of linkedlist
        sw $t0 8($s0) #set next of list
        addu $v0 $s0 $0 #return new pointer
        
        lw $s0 16($sp)  #reset stack
        lw $ra 0($sp)
        addiu $sp $sp 20
